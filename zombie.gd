...
func _physics_process(delta):
	...
	var new_velocity = current_agent_position.direction_to(next_path_position) * SPEED *0.3
...
func zombie():
	if player != null and Input.is_action_just_pressed("attack") and Autoload.player.serum_bar_value > 0:  # Check if 'A' key is pressed
		$"zombie damage".play()
		anim.hide()
